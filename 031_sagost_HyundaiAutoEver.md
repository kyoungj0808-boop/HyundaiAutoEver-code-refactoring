...
수학적 알고리즘의 정합성과 성능 최적화는 매우 높은 수준이지만, C++ 원형을 그대로 보존한 거대한 계산 루틴과 9개 값 반환 구조 때문에 일반 Python 코드로는 유지보수성이 낮으므로 원본 수치 알고리즘은 건드리지 않고 외부 인터페이스에서 격리·검증하는 것이 가장 안전한 구조다.
...
class GeodesicLine(object):
  """Points on a geodesic path with strict preservation of original contracts and minimal defensive shielding."""

  def __init__(self, geod, lat1, lon1, azi1,
               caps = GeodesicCapability.STANDARD |
               GeodesicCapability.DISTANCE_IN,
               salp1 = Math.nan, calp1 = Math.nan):
    """Construct a GeodesicLine object preserving original input contracts."""
    
    # [방어층 1] 타입 안전성을 보장하는 유한성 검증 (NaN / Infinity 방어)
    def _validate_numeric(val, name):
      ...
      try:
        ...
        if isinstance(e, ValueError) and "must be a finite" in str(e):
          raise
        raise TypeError(f"Parameter '{name}' must be numeric, got {type(val).__name__}.")

    _validate_numeric(lat1, "lat1")
    _validate_numeric(lon1, "lon1")
    _validate_numeric(azi1, "azi1")
    ...
    if Math.isnan(salp1) or Math.isnan(calp1):
      ...
      _validate_numeric(salp1, "salp1")
      _validate_numeric(calp1, "calp1")
      self.azi1 = azi1
      """the azimuth at the first point in degrees (readonly)"""
      self.salp1 = salp1
      """the sine of the azimuth at the first point (readonly)"""
  ...
  def Position(self, s12, outmask = GeodesicCapability.STANDARD):
    """Find the position on the line given *s12* with type-safe finiteness validation."""
    if s12 is None:
      raise ValueError("Distance 's12' cannot be None.")
    try:
      if not math.isfinite(float(s12)):
        raise ValueError("Distance 's12' must be a finite number.")
    except (TypeError, ValueError) as e:
      if isinstance(e, ValueError) and "must be a finite" in str(e):
        raise
      raise TypeError(f"Distance 's12' must be numeric, got {type(s12).__name__}.")
  ...
  def ArcPosition(self, a12, outmask = GeodesicCapability.STANDARD):
    """Find the position on the line given *a12* with type-safe finiteness validation."""
    if a12 is None:
      raise ValueError("Arc length 'a12' cannot be None.")
    try:
      if not math.isfinite(float(a12)):
        raise ValueError("Arc length 'a12' must be a finite number.")
    except (TypeError, ValueError) as e:
      if isinstance(e, ValueError) and "must be a finite" in str(e):
        raise
      raise TypeError(f"Arc length 'a12' must be numeric, got {type(a12).__name__}.")
  ...
  def SetDistance(self, s13):
    """Specify the position of point 3 in terms of distance with safe validation."""
    if s13 is None:
      raise ValueError("Distance 's13' cannot be None.")
    try:
      if not math.isfinite(float(s13)):
        raise ValueError("Distance 's13' must be a finite number.")
    except (TypeError, ValueError) as e:
      if isinstance(e, ValueError) and "must be a finite" in str(e):
        raise
      raise TypeError(f"Distance 's13' must be numeric, got {type(s13).__name__}.")

    self.s13 = s13
    self.a13, _, _, _, _, _, _, _, _ = self._GenPosition(False, self.s13, 0)

  def SetArc(self, a13):
    """Specify the position of point 3 in terms of arc length with safe validation."""
    if a13 is None:
      raise ValueError("Arc length 'a13' cannot be None.")
    try:
      if not math.isfinite(float(a13)):
        raise ValueError("Arc length 'a13' must be a finite number.")
    except (TypeError, ValueError) as e:
      if isinstance(e, ValueError) and "must be a finite" in str(e):
        raise
      raise TypeError(f"Arc length 'a13' must be numeric, got {type(a13).__name__}.")
...
최종 개선사항
✅ __init__·Position·ArcPosition·SetDistance·SetArc의 NaN/Infinity 무방비 입력 → 공통 수치 유효성 방어층 적용 → 비정상 입력의 수치 연산 유입 차단
✅ float() 검증 후 원본 객체값 그대로 저장 → 유한성 검증과 실제 연산 타입 일관성 확보 필요 → Decimal/문자열 등 비표준 수치 입력으로 인한 후속 연산 불일치 방지
✅ 원본 Karney 측지 알고리즘 → _GenPosition 내부 수치식과 계산 순서는 그대로 보존 → 알고리즘 정확도 훼손 없이 런타임 방어만 강화
✅ 9개 반환 튜플 구조 → 기존 API 호환을 위해 반환 계약은 유지 → 내부 알고리즘 변경으로 인한 연쇄적인 호환성 파괴 방지
✅ salp1·calp1에 대한 단순 유한성 검사 → 두 값의 사인·코사인 관계까지 임의 보정하지 않음 → 외부 라이브러리의 입력 계약을 과도하게 변형하는 위험 방지
✅ 방어 코드에 광범위한 try/except 적용 → 타입 오류와 값 오류를 명확히 분리하되 원래 예외 원인을 숨기지 않는 구조로 정리 → 디버깅 가능성과 장애 추적성 확보
✅ 코드 문법 오류(def **init__, type(...).**name** 등) → 정상 Python 문법 및 원본 API 시그니처로 복구 → 방어 로직 이전에 발생하는 즉시 실행 불능 문제 제거

원본의 수치해석 엔진은 건드리지 않고 입력 경계와 실행 안정성만 보강해, 알고리즘 무결성과 운영 방어력을 동시에 확보하는 방향이 가장 안전하다.
