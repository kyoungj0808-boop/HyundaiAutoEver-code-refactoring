...
Qt Designer 기반 UI의 기능적 골격은 유지하면서도, 방치된 데드 코드와 불필요한 이벤트 검사를 제거하고 초기화 시점의 강제 resize()를 걷어내어 Qt 레이아웃 수명주기에 맡기는 방향이 가장 안전한 개선이다.

제안패치
# coding=utf-8
"""UI form and custom Scroller widget for Video UAV Tracker - Player with precise event consumption and clean initialization."""
...
class Scroller(QtWidgets.QScrollArea):
    """비디오 뷰포트의 휠 이벤트를 제어하기 위한 커스텀 스크롤 영역."""

    def __init__(self):
        super().__init__()

    def wheelEvent(self, ev):
        """휠 이벤트가 상위로 전파되지 않도록 확실하게 이벤트를 소비(accept)합니다."""
        ev.accept()


class Ui_Form(object):
    """Video UAV Tracker 메인 플레이어 UI 클래스."""
    ...
    def setupUi(self, Form):
        ...
        # <<
        self.toolButton_11 = QtWidgets.QToolButton(self.dockWidgetContents_7)
        icon3_alt = QtGui.QIcon()
        icon3_alt.addPixmap(QtGui.QPixmap(":/VgisIcon/mActionArrowLeft.svg"), QtGui.QIcon.Normal, QtGui.QIcon.On)
        self.toolButton_11.setIcon(icon3_alt)
        self.toolButton_11.setObjectName("toolButton_11")
        self.horizontalLayout_3.addWidget(self.toolButton_11)
        ...
        # >>>
        self.toolButton_next1min = QtWidgets.QToolButton(self.dockWidgetContents_7)
        icon6_alt = QtGui.QIcon()
        icon6_alt.addPixmap(QtGui.QPixmap(":/VgisIcon/mActionArrowRight.svg"), QtGui.QIcon.Normal, QtGui.QIcon.On)
        self.toolButton_next1min.setIcon(icon6_alt)
        self.toolButton_next1min.setObjectName("toolButton_next1min")
        self.horizontalLayout_3.addWidget(self.toolButton_next1min)
...
최종 개선사항
✅ 불필요한 _old_pos 상태값 및 주석 데드 코드 → 완전 제거 → Scroller의 상태 복잡도와 유지보수 혼선 축소
✅ wheelEvent()의 무의미한 이벤트 무시 처리 → accept() 기반 명시적 이벤트 소비 → 비디오 영역의 휠 입력 계약 명확화
✅ 초기화 단계의 강제 video_frame.resize() → Qt 레이아웃/Size Policy 위임 → 레이아웃 충돌 및 초기 렌더링 불안정성 방지
✅ 구식 super(Scroller, self) 호출 → Python 3의 super() 전환 → 상속 구조 간결화 및 현대적 코드 일관성 확보
✅ 수동 UI 생성 과정의 불필요한 임시 객체 중복 → 필요한 범위에서 명칭과 생성 흐름 정돈 → UI 초기화 코드의 가독성 및 추적성 향상
✅ UI 생성 코드에 무분별한 예외 처리 추가 → Qt 기본 예외 전파 유지 → 실제 초기화 실패를 은닉하지 않고 장애 원인 추적 가능
✅ Qt Designer 생성 구조의 직접적인 기능 로직 침투 → UI 구성과 런타임 동작의 역할 분리 유지 → 재생성·확장 시 코드 무결성 확보

원본의 UI 기능과 위젯 계약은 유지하면서 이벤트 소비·레이아웃 초기화·데드 상태를 정리해, 현재 버전은 불필요한 복잡성을 제거하면서 런타임 안정성과 유지보수성을 확보한 실무형 UI 구조로 승격되었다.        
